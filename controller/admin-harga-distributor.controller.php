...
if(mysqli_num_rows($resultKomoditas) > 0) {
  while($row = mysqli_fetch_assoc($resultKomoditas)) {
    ...
    foreach($week_dates as $date) {
      $queryPermintaan = "SELECT h.id, h.id_komoditas, k.nama, h.harga, h.created_at FROM harga_distributor h JOIN komoditas k ON h.id_komoditas = k.id WHERE DATE(h.created_at) = '$date' AND h.id_komoditas = $row[id]";
