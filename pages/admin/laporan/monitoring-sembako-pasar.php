<?php
  ...
  $query = "SELECT sembako.*, pasar.nama as pasar FROM sembako JOIN pasar ON sembako.id_pasar = pasar.id WHERE sembako.deleted_at is NULL";

  if($tipeFilter == "BULANAN") {
    $query = "SELECT sembako.*, pasar.nama as pasar FROM sembako JOIN pasar ON sembako.id_pasar = pasar.id WHERE sembako.deleted_at is NULL AND MONTH(sembako.created_at) = $dataFilterBulan";
