<?php
  ...
  if($tipeFilter == "BULANAN") {
    $query = "SELECT * FROM user WHERE MONTH(user.created_at) = $dataFilterBulan";
    ...
    $query = "SELECT * FROM user WHERE ((DATE(user.created_at) BETWEEN '$dataFilterPekanAwal' AND '$dataFilterPekanAkhir') OR DATE(user.created_at) = '$dataFilterPekanAwal' OR DATE(user.created_at) = '$dataFilterPekanAkhir')";
  }
